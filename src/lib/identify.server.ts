...
function parseCuts(text: string, allowed: Set<string>): JudgeOut[] {
  ...
  for (const row of rows) {
    ...
    const item = row as { id?: unknown; category?: unknown; title?: unknown; reason?: unknown };
    ...
    out.push({ id: item.id, category, title, reason });
...
export async function judgeMomentsOnServer(moments: JudgeIn[]): Promise<JudgeResult> {
  ...
  const clipped = moments.slice(0, 6).filter((m) => m.image.length > 80 && m.image.length < 180_000);
  ...
  for (const moment of clipped) {
    content.push({ type: "text", text: moment.id });
    content.push({ type: "image_url", image_url: { url: `data:image/jpeg;base64,${moment.image}` } });
  }
